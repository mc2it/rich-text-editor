...
class TimestampEditing extends Plugin {
	...
	#createTimestampView(modelItem, viewWriter) {
		...
		const innerText = viewWriter.createText(new Date(timestamp).toLocaleString(this.editor.locale.contentLanguage, {dateStyle: "long", timeStyle: "short"}));
	...
	#defineConverters() {
		...
		this.editor.conversion.for("upcast").elementToElement({
			model: (viewElement, conversionApi) => {
				const timestamp = viewElement.hasAttribute("data-value") ? Number(viewElement.getAttribute("data-value")) : Date.now();
